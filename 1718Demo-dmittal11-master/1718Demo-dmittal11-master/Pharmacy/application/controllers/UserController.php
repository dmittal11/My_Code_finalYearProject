...
//                $data['nav'] = $this->load->view('front/include/nav', $data, true);
                ...
                $data = array(
                    ...
                    'is_confirmed' => 1,
                    'is_deleted' => 0,
                    'created_at' => date("Y-m-d H:i:s"),'title'=>'','position'=>'','company'=>''
                );
    ...
    public function addmedication() {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->form_validation->set_rules('medication_name', 'Medication Name', 'trim|required|xss_clean|is_unique[sb_pharmacy_medication.medication_name]');
          ...
          //  $this->form_validation->set_rules('use_of_direction', 'Use of direction', 'trim|required|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
             ...
             // print_r($_POST);
                $data = array(
                    'medication_name' => trim($this->input->post('medication_name')),
                    'medication_id'=>($this->input->post('medication_id') ? trim($this->input->post('medication_id') ): 0),
                    'patient_id' => trim($this->input->post('medication_patient')),
                    'quantity' => trim($this->input->post('quantity')),
                    'dosage' => trim($this->input->post('dosage')),
                    'time_duration' => trim($this->input->post('timeduration')),
                    'use_of_direction' => '','managed_repeat'=>0,'medication_status'=>0,
                    'shipping_method' => trim($this->input->post('shipping_method')),
                    'pickup_date' => $format_pickup_date,
                    'is_deliever_soon' => ($this->input->post('is_deliever_soon') ?  trim($this->input->post('is_deliever_soon')): 0),
                    'create_date' => date("Y-m-d H:i:s"),
                );
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
           ...
           // $this->form_validation->set_rules('use_of_direction', 'Use of direction', 'trim|required|xss_clean');
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $data = array(
                    'medication_name' => trim($this->input->post('medication_name')),
                    'medication_id'=>trim($this->input->post('medication_id')),
                    'patient_id' => trim($this->input->post('medication_patient')),
                    'quantity' => trim($this->input->post('quantity')),
                    'dosage' => trim($this->input->post('dosage')),
                    'time_duration' => trim($this->input->post('timeduration')),
    ...
    /*     * *********update medication*********************** */
    function check_medication_name(){
    	$name = $this->input->post('medication_name');
    	$medication_id = $this->uri->segment(2);
    	$medication_conditions =array('medication_name' => $name , 'id!='=>$medication_id);
    	$ret  =$this->Common_model->select_where('pharmacy_medication', $medication_conditions); 
    	if($ret){
    		 $this->form_validation->set_message('check_medication_name', 'Medication name must be unique');
    		return false;
    	}else{
    		return true;
    	}
...
//        print_r($data['medication']);die;
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->form_validation->set_rules('medication_name', 'Medication Name', 'trim|required|xss_clean|callback_check_medication_name');
            $this->form_validation->set_rules('quantity', 'Quantity', 'trim|required|xss_clean');
            $this->form_validation->set_rules('dosage', 'Dosage', 'trim|required|xss_clean');
            //$this->form_validation->set_rules('use_of_direction', 'Use of direction', 'trim|required|xss_clean');
            $user_id = ($this->uri->segment(2) != '') ? $this->uri->segment(2) : $this->session->userdata('user_id');
    ...
    public function getMedicaton(){
      
       $sql = "SELECT * FROM sb_pharmacy_medication WHERE medication_name like '" . $_POST["keyword"] . "%' ORDER BY medication_name  asc";
       $result =  $this->Common_model->myQuery( $sql);
       $html = '';
       if($result){
            $html .= '<ul id="country-list">';
            foreach ($result as $key => $value) {
                $funt = "onClick=selectCountry('".$value["medication_name"]."',".$value["id"].");";
                $html .= '<li '.$funt.'>'. $value["medication_name"].'</li>';
            }
            $html .= '</ul>';
    ...
    /*********************medication order stock **********************/


    public function medicationstock() {
        // send error to the view
        $data['active'] = "stock management";
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if($this->input->post('single_date')){
               $data['today'] =   $this->input->post('single_date'); 
            }else{
               $today_date = $this->input->post('today_date'); 
               $type = $this->input->post('type');
               if($type == 'pre') {
                 $new_date =  date('d-m-Y', strtotime('-1 day', strtotime($today_date)));
               }else{
                 $new_date =  date('d-m-Y', strtotime('+1 day', strtotime($today_date)));
               }
               $data['today'] = $new_date;
        ...
        $sql= "SELECT id,medication_name, (SELECT sum(quantity) FROM `sb_medication` where medication_id = a.medication_id and medication_name=a.medication_name) as stock FROM `sb_medication` as a where DATE_FORMAT(a.create_date,'%d-%m-%Y') = '".$data['today']."' GROUP by a.medication_name ,a.medication_id";
        $data['result'] =  $this->Common_model->myQuery( $sql);
        //$data['medication_list'] = $this->Common_model->select_all_where('pharmacy_medication', $conditions);
        $this->load->view('front/include/header', $data);
        $this->load->view('front/medication_stock', $data);
        $this->load->view('front/include/footer');
    }
